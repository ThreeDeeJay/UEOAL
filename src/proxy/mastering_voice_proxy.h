...
// UEOAL – IXAudio2MasteringVoice proxy (thin pass-through)
#pragma once
...
class UEOALMasteringVoice final : public IXAudio2MasteringVoice {
public:
    explicit UEOALMasteringVoice(IXAudio2MasteringVoice* real) : m_real(real) {}

    IXAudio2MasteringVoice* GetReal() const noexcept { return m_real; }
    ...
    HRESULT STDMETHODCALLTYPE GetChannelMask(DWORD* pChannelmask) override {
        return m_real->GetChannelMask(pChannelmask);
    }

private:
    IXAudio2MasteringVoice* m_real = nullptr;
};
